...
export function ClubCard({ club, isActive }: ClubCardProps) {
  return (
    <motion.div
      className="relative h-[620px] w-[360px] flex-shrink-0 cursor-grab overflow-hidden rounded-xl active:cursor-grabbing"
      initial={false}
    ...
    >
      ...
      <div className="h-[55%] bg-[#0a0a0a] px-6 py-6 flex flex-col">
        <h3 className="text-2xl font-semibold text-white tracking-tight leading-tight">{club.name}</h3>
        <p className="mt-2 text-sm text-[#888888] font-medium">{club.tagline}</p>
        <p className="mt-4 text-[13px] text-[#666666] leading-relaxed line-clamp-4">{club.description}</p>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-white/5 border border-white/5">
            ...
            <span className="text-xs text-white/60 font-medium">{club.members}</span>
          </div>
